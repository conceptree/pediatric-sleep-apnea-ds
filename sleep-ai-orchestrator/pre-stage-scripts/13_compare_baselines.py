...
def main() -> int:
    ...
    if not step11_preds.exists() or not step12_preds.exists():
        raise FileNotFoundError("Missing predictions from step 11 and/or step 12.")

    # Keyed by (record_id, split)
    ...
    out_file = outputs_dir / "baselines" / "baseline_comparison_step13.txt"
    ...
    # Winner by test balanced accuracy (primary) then test F1 (tie-breaker)
    test_rank = sorted(
        [
            (
                model_name,
