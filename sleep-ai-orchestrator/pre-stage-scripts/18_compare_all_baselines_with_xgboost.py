...
def main() -> int:
    ...
    step17_preds = outputs_dir / "baselines" / "baseline_predictions_step17_xgboost.csv"

    required = [step11_preds, step12_preds, step15_preds, step17_preds]
    ...
    with step17_preds.open("r", encoding="utf-8", newline="") as f:
        reader = csv.DictReader(f)
        for row in reader:
            key = ((row.get("record_id") or "").strip(), (row.get("split") or "").strip())
            if key in merged:
                merged[key]["pred_xgboost"] = parse_int(row.get("y_pred_xgboost"))
    ...
    out_file = outputs_dir / "baselines" / "baseline_comparison_step18_all_models_with_xgboost.txt"
    ...
    lines.append(f"step17_predictions: {step17_preds}")
    lines.append("")
