...
def main() -> int:
    try:
        ...
        raise RuntimeError(
            "This step requires numpy and scikit-learn. Install with: pip install numpy scikit-learn"
        ) from exc
    ...
    metrics_file = out_dir / "baseline_metrics_step12_logreg.txt"
    preds_file = out_dir / "baseline_predictions_step12_logreg.csv"
    ...
    # class_weight=balanced helps the minority class without altering the split strategy.
    ...
    # Tune decision threshold on validation only.
    for i in range(5, 96):
        t = i / 100.0
    ...
    lines.append("")
    lines.append("model_coefficients_standardized_space:")
    for name, coef in zip(feature_cols, model.coef_[0]):
        lines.append(f"  {name}: {coef:.6f}")
    lines.append(f"  intercept: {model.intercept_[0]:.6f}")
    ...
    with preds_file.open("w", encoding="utf-8", newline="") as f:
        ...
        for split_name, split_rows, x_s, y_true_arr in [
            ("train", train_rows, x_train_s, y_train),
            ("val", val_rows, x_val_s, y_val),
            ("test", test_rows, x_test_s, y_test),
        ]:
                ...
                writer.writerow(
                    {
                        ...
                        "y_proba_logreg": f"{float(proba[idx]):.6f}",
                        "y_pred_logreg": int(pred[idx]),
                    }
                )
